...
func (r *RabbitMQ) ConsumeMessages(queueName string, handler MessageHandler) error {
	...
	// to a service at a time.
	// The worker will only get the next message after it has
	...
	go func() {
		for msg := range msgs {
			...
			if err := handler(ctx, msg); err != nil {
				...
				// Consider a dead-letter exchange (DLQ)
				// or a more sophisticated retry mechanism for production.
