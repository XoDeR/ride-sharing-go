...
func handleTripPreview(w http.ResponseWriter, r *http.Request) {
	...
	defer r.Body.Close() // body will be closed, this is harmless but redundant
	...
	// Why we need to create a new client for each connection:
	// because if a service is down, we don't want to block the whole application
	// so we create a new client for each connection -- this will be corrected, wrong approach
	tripService, err := grpc_clients.NewTripServiceClient()
	if err != nil {
